...
DEFAULT_TOTP_STEPUP_TTL_SECONDS = _feature_configs.DEFAULT_TOTP_STEPUP_TTL_SECONDS
...
TotpStepUpPolicyMode = _feature_configs.TotpStepUpPolicyMode
...
class LitestarAuthConfig[UP: UserProtocol[Any], ID]:
    ...
    TOTP:
        ...
        ``totp_stepup_ttl_seconds`` controls the short-lived recent-TOTP marker
        TTL. ``totp_stepup_policy`` is reserved for endpoint enforcement rules
        consumed by downstream guards/controllers.
    ...
    Security and token policy:
        ...
        ``register_minimum_response_seconds``, ``verify_minimum_response_seconds``,
        ``request_verify_minimum_response_seconds``, ``deployment_worker_count``, ``id_parser``,
        ``superuser_role_name``.
        ...
        ``verify_minimum_response_seconds`` and
        ``request_verify_minimum_response_seconds`` apply the same response
        floor to plugin-owned verification endpoints.
    ...
    def _validate_totp_stepup_configuration(self) -> None:
