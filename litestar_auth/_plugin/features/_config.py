...
class OrganizationConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.tenant_resolver is None:
            self.tenant_resolver = HeaderTenantResolver(header_name=self.tenant_header_name)


@dataclass(slots=True)
class DatabaseTokenAuthConfig:
    """DB-token secure-cookie preset settings owned by ``LitestarAuthConfig``."""
    ...
    cookie: CookieTransportConfig = field(default_factory=CookieTransportConfig)
