...
def _validate_api_key_policy_fields(api_key_config: ApiKeyConfig) -> None:
    ...
    if api_key_config.max_keys_per_user <= 0:
        msg = "api_keys.max_keys_per_user must be greater than 0."
        raise ConfigurationError(msg)
    if api_key_config.scope_subset_check and not api_key_config.allowed_scopes:
        msg = "api_keys.allowed_scopes must be non-empty when scope_subset_check is True."
        raise ConfigurationError(msg)
    if api_key_config.last_used_throttle_seconds < 0:
        msg = "api_keys.last_used_throttle_seconds must be non-negative."
        raise ConfigurationError(msg)
    if api_key_config.last_used_write_strategy not in {"disabled", "immediate", "throttled"}:
        msg = "api_keys.last_used_write_strategy must be 'disabled', 'immediate', or 'throttled'."
        raise ConfigurationError(msg)
    if api_key_config.signing_skew_seconds < 1:
        msg = "api_keys.signing_skew_seconds must be greater than 0."
        raise ConfigurationError(msg)
    if api_key_config.signed_body_max_bytes < 1:
        msg = "api_keys.signed_body_max_bytes must be greater than 0."
        raise ConfigurationError(msg)
    if api_key_config.signed_body_max_messages < 1:
        msg = "api_keys.signed_body_max_messages must be greater than 0."
        raise ConfigurationError(msg)
