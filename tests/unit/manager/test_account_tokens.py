...
from litestar_auth.manager import RESET_PASSWORD_TOKEN_AUDIENCE, VERIFY_TOKEN_AUDIENCE, _get_dummy_hash
...
async def test_forgot_password_existing_user_calls_hook_with_valid_reset_token() -> None:
    ...
    await manager._account_tokens.forgot_password("User@Example.com", dummy_hash=_get_dummy_hash())
    ...
    assert payload["password_fingerprint"] == manager._password_fingerprint(user.hashed_password)
...
async def test_reset_password_updates_hash_invalidates_tokens_and_calls_hook() -> None:
    ...
    token = manager._account_tokens.write_reset_password_token(user, dummy_hash=_get_dummy_hash())
    user_db.get.return_value = user
...
async def test_reset_password_rejects_stale_password_fingerprint() -> None:
    ...
    token = manager._account_tokens.write_reset_password_token(original_user, dummy_hash=_get_dummy_hash())
...
def test_password_fingerprint_matches_expected_hmac() -> None:
    ...
    assert fingerprint == manager._password_fingerprint(hashed_password)
