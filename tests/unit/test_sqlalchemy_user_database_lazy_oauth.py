...
def test_sqlalchemy_user_database_supports_password_column_name_hook_without_reference_models() -> None:
    ...
    code = (
        ...
        "class AppUUIDBase(UUIDPrimaryKey, AppBase):\n"
        "    __abstract__ = True\n"
        "class PasswordColumnUser(UserModelMixin, UserAuthRelationshipMixin, AppUUIDBase):\n"
        "    __tablename__ = 'password_column_user'\n"
        "    auth_access_token_model = None\n"
        "    auth_refresh_token_model = None\n"
        "    auth_oauth_account_model = 'PasswordColumnOAuthAccount'\n"
        "    auth_hashed_password_column_name = 'password_hash'\n"
        "class PasswordColumnOAuthAccount(OAuthAccountMixin, AppUUIDBase):\n"
        "    __tablename__ = 'password_column_oauth_account'\n"
        "    auth_user_model = 'PasswordColumnUser'\n"
        "    auth_user_table = 'password_column_user'\n"
        "database = SQLAlchemyUserDatabase(\n"
        "    session=MagicMock(),\n"
        "    user_model=PasswordColumnUser,\n"
        "    oauth_account_model=PasswordColumnOAuthAccount,\n"
        ")\n"
        "assert database.user_model is PasswordColumnUser\n"
        "assert database._require_oauth_account_model() is PasswordColumnOAuthAccount\n"
        "assert inspect(PasswordColumnUser).attrs.hashed_password.columns[0].name == 'password_hash'\n"
        "assert inspect(PasswordColumnOAuthAccount).relationships['user'].mapper.class_ is PasswordColumnUser\n"
        "assert 'litestar_auth.models.user' not in sys.modules\n"
        "assert 'litestar_auth.models.oauth' not in sys.modules\n"
    )
    result = subprocess.run(
