...
def _exceptions_module() -> ModuleType:
    """Import the runtime exceptions module lazily so reload-based tests stay stable.

    Returns:
        The current ``litestar_auth.exceptions`` module object.
    """
    return importlib.import_module("litestar_auth.exceptions")


def _invalid_password_error_cls() -> type[Exception]:
    """Return the current runtime InvalidPasswordError class."""
    return cast("type[Exception]", _exceptions_module().InvalidPasswordError)


def _inactive_user_error_cls() -> type[Exception]:
    """Return the current runtime InactiveUserError class."""
    return cast("type[Exception]", _exceptions_module().InactiveUserError)


def _unverified_user_error_cls() -> type[Exception]:
    """Return the current runtime UnverifiedUserError class."""
    return cast("type[Exception]", _exceptions_module().UnverifiedUserError)
