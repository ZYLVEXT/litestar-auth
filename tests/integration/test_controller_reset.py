...
async def test_reset_password_updates_hash_for_valid_token(
    ...
    user = ExampleUser(
        ...
        hashed_password=password_helper.hash("old-valid-password"),
        is_verified=True,
        roles=["member"],
    )
    user_db.users_by_id[user.id] = user
    ...
    response = await test_client.post("/auth/reset-password", json={"token": token, "password": "new-valid-password"})
    ...
    assert password_helper.verify("new-valid-password", stored_user.hashed_password) is True
...
async def test_reset_password_token_invalid_after_password_change(
    ...
    updated_user = ExampleUser(
        id=user.id,
        email=user.email,
        hashed_password=password_helper.hash("other-valid-password"),
    ...
    response = await test_client.post(
        "/auth/reset-password",
        json={"token": token, "password": "new-valid-password"},
    )
...
async def test_reset_password_rejects_invalid_and_expired_tokens() -> None:
    ...
    async with AsyncTestClient(app=app) as client:
        invalid_response = await client.post(
            "/auth/reset-password",
            json={"token": "not-a-valid-token", "password": "new-valid-password"},
        )
        expired_response = await client.post(
            "/auth/reset-password",
            json={"token": expired_token, "password": "new-valid-password"},
        )
...
async def test_reset_password_rejects_short_password_at_schema_boundary(
    client: tuple[AsyncTestClient[Litestar], InMemoryUserDatabase, TrackingUserManager],
) -> None:
    """Reset-password publishes and enforces the new-password floor before token use."""
    test_client, user_db, user_manager = client
    password_helper = PasswordHelper()
    user = ExampleUser(
        id=uuid4(),
    ...
    assert retry.status_code == HTTP_OK
...
async def test_reset_password_rejects_malformed_json_with_controller_error_contract() -> None:
    """Reset-password keeps the controller 400 malformed-body error contract."""
...
async def test_reset_password_rejects_token_longer_than_2048_characters() -> None:
    ...
    async with AsyncTestClient(app=app) as client:
        response = await client.post(
            "/auth/reset-password",
            json={"token": "t" * 2049, "password": "new-valid-password"},
        )
...
async def test_reset_password_supports_custom_user_read_schema() -> None:
    ...
    async with AsyncTestClient(app=app) as client:
        response = await client.post("/auth/reset-password", json={"token": token, "password": "new-valid-password"})
...
async def test_reset_password_rate_limit_returns_429_after_repeated_invalid_attempts() -> None:
    ...
    async with AsyncTestClient(app=app) as client:
        ...
        second_response = await client.post(
            "/auth/reset-password",
            json={"token": "also-not-valid", "password": "new-valid-password"},
        )
        blocked_response = await client.post(
            "/auth/reset-password",
            json={"token": "still-not-valid", "password": "new-valid-password"},
        )
