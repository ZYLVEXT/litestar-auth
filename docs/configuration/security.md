...
Timing floors and lockout/rate-limit windows must be finite numbers. `NaN` and positive or negative
infinity are rejected during construction so timing padding cannot be disabled and backend TTLs
cannot become unbounded or invalid.
...
- `JWTStrategy(secret=..., allow_inmemory_denylist=True)` reports `revocation_posture.key == "in_memory"` and `revocation_is_durable == False`.
  `InMemoryJWTDenylistStore` refuses new revocations when its `max_entries` cap is hit and no expired JTIs can be pruned: the failed logout returns 503 instead of pretending the still-valid token was revoked, and existing active revocation entries are preserved.
...
- `LitestarAuthConfig(account_token_denylist_store=RedisJWTDenylistStore(...))` (or the same `account_token_denylist_store=` argument on `BaseUserManager`) makes verify and reset-password tokens atomically single-use server-side. The store implements `JWTReplayStore.mark_used()`, which consumes an unseen `jti` after non-mutating validation and immediately before mutation, and rejects concurrent replays. Capacity pressure produces 503 without mutation. The same Redis object can back access-token revocation, but custom stores must implement both the ordinary `JWTDenylistStore` methods and atomic `mark_used()`. As with JWT revocation posture, custom stores are assumed shared unless they expose `revocation_is_durable=False`; process-local custom stores must set that marker so topology validation can reject them in multi-worker deployments. When unset, account-token single-use rests on fingerprint/verification-state rotation and does not provide cross-worker atomicity; plugin startup emits a `SecurityWarning`. With `deployment_worker_count > 1`, configuring a process-local replay store fails startup because it would provide misleading per-worker protection.
