...
| `include_oauth_associate` | `False` | Also auto-mount authenticated associate routes for the same `oauth_providers` inventory under `POST {auth_path}/associate/{provider}/authorize` and `GET {auth_path}/associate/{provider}/callback`. The authorize route is POST + CSRF-protected to defeat forced-association attacks; see [Cookbook: OAuth associate](../cookbook/oauth_associate.md) for the browser-flow recipe. |
...
| `oauth_redirect_dns_strict` | `True` | Fail-closed DNS validation for OAuth redirect hosts (default). DNS resolver failures or empty/unusable resolver answers reject the redirect base with `ConfigurationError`. Set `False` to restore fail-open behavior for offline or sandboxed startup environments where DNS is unavailable. This is startup/controller-construction defense-in-depth only; it does not defend against DNS rebinding. |
...
- `oauth_redirect_dns_strict` defaults to `True`, so redirect-host DNS validation fails closed when the resolver cannot return a usable public address. Set it to `False` to restore fail-open DNS behavior in offline or sandboxed startup environments.
...
- Redirect-host DNS is resolved once during plugin startup or manual controller construction. The validation rejects internal-only answers such as loopback, RFC 1918 private, RFC 3927 link-local including `169.254.169.254`, multicast, reserved, or unspecified addresses, but it cannot prove that later runtime provider callbacks will resolve to the same address. Treat DNS rebinding as out of scope and enforce deployment-owned egress controls that block application traffic to RFC 1918, link-local, metadata-service, and other internal networks.
