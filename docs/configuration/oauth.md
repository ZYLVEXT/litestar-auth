...
| `oauth_redirect_dns_strict` | `False` | Opt-in fail-closed DNS validation for OAuth redirect hosts. When `True`, DNS resolver failures or empty/unusable resolver answers reject the redirect base with `ConfigurationError`; the default preserves the historical fail-open behavior. This is startup/controller-construction defense-in-depth only; it does not defend against DNS rebinding. |
...
- `oauth_redirect_dns_strict=True` makes redirect-host DNS validation fail closed when the resolver cannot return a usable public address. Leave it unset to preserve the default fail-open DNS behavior in offline or sandboxed startup environments.
