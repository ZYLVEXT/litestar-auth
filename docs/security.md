...
- **Reset & verify tokens** — signed JWT-style tokens with a password fingerprint so reset tokens die after password change (de-facto single-use). Library-issued verify and reset tokens include JOSE `typ=JWT`; missing or unexpected `typ` headers are rejected before the normal signed decode. For explicit server-side single-use, configure `account_token_denylist_store` with a shared `JWTReplayStore` such as `RedisJWTDenylistStore` on `LitestarAuthConfig` or `BaseUserManager`. Its atomic `mark_used()` operation consumes the token's `jti` after all non-mutating validation and immediately **before** account mutation, so concurrent requests cannot both verify an account or reset its password while validation failures do not burn a valid token. Replay is returned as the normal invalid-token response; store capacity pressure fails closed with HTTP **503** and no account mutation. When unset (default), single-use rests on fingerprint/verification-state rotation only and cannot provide the same cross-worker atomicity; production startup emits a `SecurityWarning`. A configured process-local replay store is rejected when `deployment_worker_count > 1`.
- **JWT** — library-issued access tokens include JOSE `typ=JWT`, and access-token decode rejects missing or unexpected `typ` headers before signature, audience, issuer, algorithm, and required-claim validation. This is defense-in-depth against token-class confusion, not a substitute for those primary controls. Access-token validation also enforces `exp` / `iat` / `aud`; optional `iss`; a small `exp` / `nbf` leeway for ordinary clock skew; and `jti` denylist support (`InMemoryJWTDenylistStore`, `RedisJWTDenylistStore`) with an explicit `JWTStrategy.revocation_posture` contract. The in-memory denylist prunes expired JTIs on each revoke and refuses a new revocation when `max_entries` is reached without evicting an active entry. `JWTStrategy.destroy_token` turns that refusal into `TokenError`, and `AuthenticationBackend.logout` maps it to HTTP **503** with `TOKEN_PROCESSING_FAILED`; the token remains valid, so clients and operators must not treat the failed logout as revocation.
...
- **API keys** — opt-in user-owned credentials with digest-only bearer storage, one-time raw-secret
  ...
  or cross the non-API-key user-session boundary. Optional
...
- **Built-in global administration** — core user administration plus the opt-in role and organization administration controllers require both `is_superuser` and `requires_password_session` by default. Delegated API keys therefore cannot inherit an owning superuser's unrestricted administrative authority. Applications replacing contrib-controller guards assume responsibility for enforcing an equivalent operator boundary.
- **Session/device management** — the opt-in refresh-session inventory and revocation routes require `requires_password_session`; a delegated API key cannot inspect or terminate the owner's interactive sessions.
- **Organization switching** — the opt-in route that mints an organization-bound JWT requires `requires_password_session`; an API key cannot exchange delegated scope for a full JWT session.
- **Organization invitations** — accept/decline routes require an active, verified non-API-key user session; delegated API keys cannot change the owner's memberships or consume invitation evidence.
...
## Security log events {#security-log-events}

The following security detections are emitted as structured log records carrying an `event` key in
`extra`, so they can be routed without matching on message text. This table covers attack and
security-store degradation events; ordinary lifecycle telemetry such as successful login and
registration is intentionally outside its scope.

| Event | Logger | Level | Extra fields | Meaning |
|---|---|---|---|---|
| `refresh_token_reuse_detected` | `litestar_auth.security` | `WARNING` | `session_id` | A consumed refresh-token digest was presented again. The whole session chain has been revoked (RFC 6819 5.2.2.3). Treat as a credible token-theft signal. |
| `api_key_nonce_replay` | `litestar_auth.security` | `WARNING` | `key_id` | A signed API-key request reused a nonce. The request was rejected. |
| `api_key_nonce_store_capacity` | `litestar_auth.security` | `WARNING` | `key_id` | The nonce store could not record a nonce and the request failed **closed**. Infrastructure failure, not an attack — legitimate signed requests are being rejected. |
| `jwt_denylist_capacity_exceeded` | `litestar_auth.security` | `ERROR` | `max_entries` | `InMemoryJWTDenylistStore` is full of still-valid revocations and refused a new one, so a revoked token stays accepted. Move to `RedisJWTDenylistStore`. |
| `account_token_replay` | manager logger | `WARNING` | — | A verify/reset JTI was already consumed; the request was rejected before account mutation. |
| `account_token_replay_store_capacity` | manager logger | `ERROR` | — | The account-token replay store could not record a new JTI. Verify/reset failed closed with 503 before mutation. |
| `account_state_failure` | `litestar_auth.security` | `INFO` | `reason` | Correlates the internal `inactive` / `unverified` reason behind an opaque `LOGIN_ACCOUNT_UNAVAILABLE`. |
| `totp_replay` | `litestar_auth.totp` | `WARNING` | `user_id` | A TOTP code was submitted twice in the same window. |
| `totp_replay_store_capacity` | `litestar_auth.totp` | `WARNING` | `user_id` | The used-code store rejected verification under capacity pressure and MFA failed **closed**. Infrastructure failure — users are being locked out. |
| `totp_failed` | `litestar_auth.totp` | `WARNING` | `user_id` | A TOTP code did not verify. |
| `totp_enrollment_store_capacity` | `litestar_auth._totp_stores` | `WARNING` | `max_entries` | Pending TOTP enrollment could not be recorded under in-memory capacity pressure. |
| `account_lockout_memory_capacity` | `litestar_auth.ratelimit._memory` | `WARNING` | `max_keys` | The in-memory lockout store rejected a new identity and authentication failed closed. |
| `rate_limit_memory_capacity` | `litestar_auth.ratelimit._memory` | `WARNING` | `max_keys` | The in-memory request limiter rejected a new identity and the request failed closed. |
| `token_validation_failed` | component logger | `WARNING` | — | A token failed structural, subject, fingerprint, or replay validation. |
| `totp_pending_client_binding_disabled` | `litestar_auth.controllers.totp_context` | `WARNING` | — | Pending-login client binding was explicitly disabled. |

Pair each attack event with its infrastructure counterpart on separate alert routes:
`api_key_nonce_replay` and `totp_replay` mean someone is replaying credentials, while
`api_key_nonce_store_capacity`, `totp_replay_store_capacity`, `account_token_replay_store_capacity`,
and the in-memory rate-limit/lockout capacity events mean a store is degraded and legitimate traffic
is being denied. `jwt_denylist_capacity_exceeded` is different: logout fails with 503, but the token
remains valid because the revocation was not stored.
