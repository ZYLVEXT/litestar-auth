...
- `RedisAuthPreset` builds `AuthRateLimitConfig`, `RedisTotpEnrollmentStore`,
  `RedisUsedTotpCodeStore`, and the pending-token `RedisJWTDenylistStore` helper from one shared client and per-group rate-limit tiers. Import
  `AuthRateLimitSlot` from `litestar_auth.ratelimit` when calling
  `build_rate_limit_config(...)` with `enabled=...` or `disabled=...`. The shared-client contract covers the combined operations used by
