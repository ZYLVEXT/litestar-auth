...
every configured TOTP Fernet key distinct on `UserManagerSecurity`. Outside testing,
`BaseUserManager(...)` raises `ConfigurationError` when one configured value is reused across those
roles (as resolved from the `security` bundle). Distinct audiences (`litestar-auth:verify` and
