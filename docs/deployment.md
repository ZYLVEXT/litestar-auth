...
- Keep **`verification_token_secret`**, **`reset_password_token_secret`**,
  **`totp_pending_secret`**, every configured TOTP Fernet key, and **`oauth_flow_cookie_secret`** distinct. Production configuration now
  ...
  TOTP Fernet keys and `oauth_flow_cookie_secret` should remain dedicated encryption keys with no JWT audience.
