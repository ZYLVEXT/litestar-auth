"""Require immutable GitHub Actions and exact Docker reference image pins."""
...
def _invalid_image_pins(repository_root: Path, observed: dict[str, tuple[str, str, str]]) -> list[str]:
    errors: list[str] = []
    compose_files = sorted((repository_root / "docker").glob("**/compose*.y*ml"))
...
def invalid_dependency_pins(repository_root: Path) -> list[str]:
    ...
    Returns:
        Human-readable file and line diagnostics.
    """
    observed: dict[str, tuple[str, str, str]] = {}
